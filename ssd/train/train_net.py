...
from dataset.txt_format import TxtFormat
...
def load_others(name, list_file, image_dir, label_dir, shuffle=True):
    #TODO:
    classes = ["background", "face"]
    imdbs = []
    imdbs.append(TxtFormat(name, classes, list_file, image_dir, label_dir))
    return imdbs[0]
...
def train_net_on_others(net, train_set, train_list, image_dir, label_dir, batch_size,
               data_shape, mean_pixels, resume, finetune, pretrained, epoch, prefix,
               ctx, begin_epoch, end_epoch, frequent, learning_rate, momentum,
               weight_decay, val_set, val_list, lr_refactor_epoch, lr_refactor_ratio,
    ...
    # load dataset
    imdb = load_others(train_set, train_list, image_dir, label_dir)
    val_imdb = load_others(val_set, val_list, image_dir, label_dir)
